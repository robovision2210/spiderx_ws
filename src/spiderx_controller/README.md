...
gz_ros2_control) and the **M2 simulation-only CAD neutral posture-hold test**. No balance control,
gait, IK or hardware support (see `../../STATUS.md`).
...
It is not diff-drive: a quadruped needs joint control (M1) → simulation posture hold (M2) → IK → gait → `/cmd_vel` bridge.
